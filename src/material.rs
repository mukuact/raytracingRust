...
impl Scatter for Metal {
    fn scatter(&self, r_in: &Ray, rec: &HitRecord) -> Option<(Color, Ray)> {
        ...
        if scatterd.direction().dot(rec.normal) > 0.0 {
            Some((self.albedo, scatterd))
        } else {
            None
...
impl Scatter for Dielectric {
    fn scatter(&self, f_in: &Ray, rec: &HitRecord) -> Option<(Color, Ray)> {
        ...
        let unit_direction = f_in.direction().normalized();
        ...
        let scatterd = Ray::new(rec.p, direction);

        Some((Color::new(1.0, 1.0, 1.0), scatterd))
    }
}
